class Parser:
    ...
    def parseExpression(self, expression: str) -> list:
        try:
            wordToNumber = {
            ...
            def wordToNum(expression):
                ...
                while i < len(words):
                    ...
                    if word in wordToNumber:
                        tokens.append(wordToNumber[word])
                    elif word.startswith("veinti") and len(word) > 6:
                        base = 20  # Hardcoded base value for "veinti"
                        suffix = word[6:]
                        if suffix in wordToNumber:
                            tokens.append(base + wordToNumber[suffix])
                    elif word in tensToNumber:
                        # Check for compound numbers like "cuarenta y dos"
                        if i + 2 < len(words) and words[i + 1] == "y" and words[i + 2] in wordToNumber:
                            tokens.append(tensToNumber[word] + wordToNumber[words[i + 2]])
                            i += 2  # Skip "y" and the unit word
                        else:
                            tokens.append(tensToNumber[word])
